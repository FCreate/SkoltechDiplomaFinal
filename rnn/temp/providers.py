...
class SpecialTokenWrapperModel():
    ...
    def __init__(self, model,padding):
        ...
        self.special_tokens = {1:"<bos>", 2:"<eos>"}
        #self.shift = self.model.
        if hasattr(self.model,'vocab'): #Like SmilesModel
            self.vocabulary_size = self.model.vocsize + 2
        elif hasattr(self.model,'rules'): #Like Legogram
            self.vocabulary_size = len(self.model.rules) + 2
...
"""class MolecularNotationDataset(Dataset):
    def __init__(self, smiles, model):
        ...
        self.encoded = map_parallel(self.smiles, lambda s:([special_tokens["<bos>"]]+ model.encode(s) + [special_tokens["<eos>"]]),nworkers=6)
    def __len__(self):
        return len(self.encoded)

    def __getitem__(self, idx):
        return torch.LongTensor(self.encoded[idx])"""
...
class MolecularNotationDataset(Dataset):
    def __init__(self, smiles, model, data):
        ...
        self.encoded = map_parallel(self.smiles_data,lambda s:(np.array([self.vocsize-2] + model.encode(s[0]) + [self.vocsize-1])),nworkers=6)
    def __len__(self):
        return len(self.encoded)
